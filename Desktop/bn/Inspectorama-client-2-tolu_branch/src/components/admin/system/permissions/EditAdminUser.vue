<template>
  <div>
    <div class="body">
      <div class="container">
        <div class="container-header">Edit Admin User</div>
          ...
          <table>
            ...
            <tr :class="{invalid: $v.adminUser.currentAdminPassword.$error}">
              ...
              <td class="input-text">
                ...
                >Current Admin Password is required.</span>
              </td>
            </tr>
            ...
            <tr :class="{invalid: $v.adminUser.isActive.$error}">
              <td>
                <label>This&nbsp;acount&nbsp;is</label>
              </td>
              <td class="small-input-text">
                <select v-model="adminUser.isActive" required>
                  <option value selected disabled hidden>Select one</option>
                  <option value="true">Active</option>
                  <option value="false">Inactive</option>
                </select>
                <span
                  v-if="!$v.adminUser.isActive.required && $v.adminUser.isActive.$error"
...
export default {
  ...
  methods: {
    onSubmit() {
      ...
      this.$store.dispatch(
        "adminStore/updateSpecificAdminUserInfo",
        adminUserData
      );
    }
  },
  ...
  created() {
    const id = localStorage.getItem("openedUserId");
    axios
      .get(`/admin_users/${id}`)
      .then(res => {
        this.adminUser.firstName = res.data.firstName;
        this.adminUser.lastName = res.data.lastName;
        this.adminUser.username = res.data.username;
        this.adminUser.email = res.data.email;
        this.adminUser.mobileNumber = res.data.mobileNumber;
        this.adminUser.role = res.data.role;
        this.adminUser.isActive = res.data.isActive;
