  ...
  <div>
    <div class="container">
      <div class="sequence-title">
        ...
        <span>
          <router-link to="/help/general-help" tag="div">{{userType | toCapitalizeFirstLetter}}</router-link>
        </span>
        <i class="fa fa-angle-right"></i>
        <span>
      ...
      <div class="container-content">
        ...
        <div class="container-content-right">
          <div v-for="(article, index) in articlesList" :key="index">
            <p @click="onSelectArticle(article.title)">{{article.title}}</p>
          </div>
        </div>
      </div>
    </div>
...
export default {
  ...
  methods: {
      ...
      this.helpArticles.forEach(item => {
        item.topics.forEach(element => {
          if (element.topic == this.selectedTopic) {
            this.articlesList = element.articles;
          }
        });
      });
    },
    ...
    onSelectArticle(title) {
      this.$router.push(
        `/${this.$i18n.locale}/help/${this.userType}/${
          this.selectedCategory
        }/topic/${this.selectedTopic}/article/${encodeURIComponent(title)}`
      );
    }
  },
