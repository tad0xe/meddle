<template>
  <div>
    <div class="container">
      <div>
        <h1>{{$t('inspectorHelpLanding.mainHeading')}}</h1>
      </div>
        ...
        <div class="Search-container">
          <div class="Search-container-content">
            ...
            <input
              type="text"
              name="Search"
              :placeholder="$t('inspectorHelpLanding.searchBarPlaceholder')"
              v-model="q"
      ...
      <div>
        <h1>{{$t('inspectorHelpLanding.mainTopic')}}</h1>
      </div>
...
export default {
  ...
  navigateToHelpTopic(category, topic) {
    this.$router.push(`/${this.$i18n.locale}/help/inspector/${category}/topic/${topic}`);
  },
  getSearchResults() {
    if (this.q == "") {
      return;
    }
    this.$router.push(`/${this.$i18n.locale}/help/search-results?q=${this.q}`);
  }
  },
  created() {
    inspectorHelpArticles = this.$t('inspectorHelpArticles')
    this.articleTopicsArray = inspectorHelpArticles.map(item => {
      const newItem = {};
