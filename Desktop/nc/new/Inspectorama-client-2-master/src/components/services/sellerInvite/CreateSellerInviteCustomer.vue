    ...
    <div class="body">
      <div class="container">
        <div class="header-container flex">
          <h1 class="flex-grow">{{$t("createSellerInviteCustomer.title")}}</h1>
          <div>
            <button
              class="primary-btn"
              type="button"
        ...
        <div class="vehicle-home-description-container">
          <div class="row">
            <div v-if="invitationInfo.inspectionType == 'vehicle'">
              <h3>{{$t("generic.vehicleDescription")}}</h3>
              <div class="row">
            ...
            <div
              :class="{
                ...
                invalid: $v.invitationInfo.inspectionDateTime.$error
                }"
            >
          ...
          <div>
            ...
            <!-- agreedOrListedPrice:{{agreedOrListedPrice.value}}
            <br />
            hasSellerAgreedToRefund:{{invitationInfo.hasSellerAgreedToRefund}}-->
            <div
              class="row spaced"
        ...
        <div class="row">
          <div class="btn-container">
            <button
              class="secondary-btn"
              type="submit"
              @click.prevent="onSubmitInvite()"
            >{{$t("buttons.sendInvite")}}</button>
          </div>
        </div>
      </div>
    </div>
...
export default {
  ...
  watch: {
    //Because requiredUnless above does not work properly
    "invitationInfo.hasSellerAgreedToRefund"(to, fro) {
      if (to == false) {
        this.agreedOrListedPrice.value = null;
      }
    },
  ...
  methods: {
    onSubmitInvite() {
      ...
      axios
        .post("/order_invitation/create_order_invite", invitationInfoData)
        .then(res => {
          this.$router.push(
